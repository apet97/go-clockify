...
func printHelp() {
	...
	_, _ = fmt.Fprintln(w, "  CLOCKIFY_BASE_URL      optional, defaults to https://api.clockify.me/api/v1; overrides must be a documented Clockify host or loopback")
	_, _ = fmt.Fprintln(w, "  CLOCKIFY_ALLOW_CUSTOM_BASE_URL     optional, defaults to false; true permits an arbitrary HTTPS CLOCKIFY_BASE_URL host (logs a warning)")
