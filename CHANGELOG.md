...
- **Unified Clockify OpenAPI artifact.** Added
  `docs/openapi/clockify-openapi.yaml`, generated by
  `scripts/gen-clockify-openapi`, with provenance-aware coverage from
  `realOPENAPI`, probe-lab fragments, pasted documentation Markdown,
  live probe findings, and the MCP catalog.
- **OpenAPI drift gates.** Added `make gen-openapi` and
  `make openapi-drift` so the generated API contract can be refreshed
  deterministically and checked in CI-style local release gates.

### Changed

- **Probe-lab documented-route coverage now tracks the refreshed API
  surface.** Expanded the `clockify_probe_lab_api` allowlist and live
  probes for workspace settings, time-off, policies, user groups,
  expense files, webhook token/logs, and documented tag CRUD routes.
- **Custom-field MCP writes match live Clockify behavior.** Update
  calls preserve the server-required custom-field type, project custom
  fields use the live `defaultValue` PATCH shape, and time-entry custom
  fields merge into full-entry updates with ownership guards intact.
- **Live validation is less workspace-state fragile.** The broad live
  suite now avoids fixed webhook-name collisions, handles local
  role-strip safety refusals, and finds an empty timesheet-fill window
  before writing.
