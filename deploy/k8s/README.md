...
| `prometheus-rule.yaml` | `PrometheusRule` carrying the SLO + transport alerts: multi-window burn-rate (99.9% SLO), `ClockifyMCPUpstreamUnavailable`, `ClockifyMCPHighToolErrorRate`, `ClockifyMCPRateLimitSaturation`, `ClockifyMCPHTTPAdmissionRejections`, `ClockifyMCPAuditFailure`, `ClockifyMCPHighLatency`, `ClockifyMCPNotReady`, `ClockifyMCPAuthFailures`, `ClockifyMCPStreamableEventsAliasUse`. The file itself is the canonical definition. |
