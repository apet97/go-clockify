...
func TestWriteResult(t *testing.T) {
	id := "abc-123"
	warnings := []string{"field X deprecated"}
	data := map[string]string{"name": "Test"}

	result := WriteResult("create", "project", &id, data, warnings)

	if result["action"] != "create" {
		t.Fatalf("expected action create, got %v", result["action"])
	}
	if result["resource_type"] != "project" {
		t.Fatalf("expected resource_type project, got %v", result["resource_type"])
	}
	if result["resource_id"] != "abc-123" {
		t.Fatalf("expected resource_id abc-123, got %v", result["resource_id"])
	}
	if result["data"] == nil {
		t.Fatal("expected data")
	}
	w, ok := result["warnings"].([]string)
	if !ok || len(w) != 1 {
		t.Fatalf("expected 1 warning, got %v", result["warnings"])
	}
}

func TestWriteResultNoWarnings(t *testing.T) {
	id := "abc-123"
	result := WriteResult("update", "project", &id, nil, nil)

	if _, exists := result["warnings"]; exists {
		t.Fatal("warnings should be omitted when empty")
	}
}

func TestWriteResultNoResourceID(t *testing.T) {
	result := WriteResult("delete", "project", nil, nil, nil)

	if _, exists := result["resource_id"]; exists {
		t.Fatal("resource_id should be omitted when nil")
	}
}
