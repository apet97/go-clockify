...
func AllSpecs() []EnvSpec {
	return []EnvSpec{
		...
		{Name: "CLOCKIFY_DOCUMENTED_API_WRITES", Group: "Core", Enum: []string{"0", "1"}, Default: "enabled locally, disabled for hosted profiles", Help: "Enable probe_lab_api generic documented POST/PUT/PATCH/DELETE calls. Hosted/shared-service profiles default to disabled so raw writes require explicit operator opt-in; read-only documented API calls stay available."},
		...
		{Name: "CLOCKIFY_CONFIRMATION_REPLAY_PROTECTION", Group: "Safety", Enum: []string{"enabled", "disabled"}, Default: "enabled on hosted profiles, disabled otherwise", Help: "When enabled, accepted confirmation-token nonces are recorded and a second use within TTL is rejected as a replay. Hosted/shared-service profiles default to enabled; local profiles may opt in with an in-memory replay cache."},
		...
		{Name: "CLOCKIFY_PER_TENANT_CONCURRENCY", Group: "Performance", Default: "0 (shared-service/prod-postgres profiles: 10)", AppliesTo: []string{"http", "streamable_http", "grpc"}, Help: "Aggregate concurrent tool-call budget per tenant. Stacks above the per tenant+subject layer so one tenant cannot consume the whole upstream Clockify budget through many subjects (0=disabled)."},
		{Name: "CLOCKIFY_PER_TENANT_RATE_LIMIT", Group: "Performance", Default: "0 (shared-service/prod-postgres profiles: 120)", AppliesTo: []string{"http", "streamable_http", "grpc"}, Help: "Aggregate tool calls per 60s window per tenant. Stacks above CLOCKIFY_PER_TOKEN_RATE_LIMIT; hosted profiles set this by default as an in-process budget while gateway/load-balancer quotas remain authoritative across replicas (0=disabled)."},
		{Name: "CLOCKIFY_TOKEN_BUDGET", Group: "Performance", Default: "24000", Help: "Response token budget (0=disabled)"},
		...
		{Name: "CLOCKIFY_CIRCUIT_BREAKER", Group: "Performance", Enum: []string{"auto", "enabled", "disabled"}, Default: "auto", Help: "Per-endpoint upstream 5xx circuit breaker. auto enables the breaker for hosted/shared-service profiles and disables it for local profiles."},
		{Name: "CLOCKIFY_CIRCUIT_BREAKER_FAILURE_THRESHOLD", Group: "Performance", Default: "5", Help: "Consecutive final upstream 5xx outcomes before a per-endpoint breaker opens [1,100]. Retries still run inside the breaker; only final outcomes count."},
		{Name: "CLOCKIFY_CIRCUIT_BREAKER_OPEN_DURATION", Group: "Performance", Default: "45s", Help: "How long an open per-endpoint upstream breaker fast-fails before allowing half-open probes [1s,10m]."},
		{Name: "CLOCKIFY_CIRCUIT_BREAKER_HALF_OPEN_PROBES", Group: "Performance", Default: "1", Help: "Concurrent half-open probe calls allowed per endpoint before local fast-fail resumes [1,10]."},
		...
		{Name: "MCP_MAX_SESSIONS_PER_REPLICA", Group: "Transport", Default: "0", AppliesTo: []string{"streamable_http"}, Help: "Streamable HTTP session cap per replica (0=disabled; initialize requests beyond the cap return 503 with Retry-After). Pair with a per-principal cap to prevent a single tenant from starving the pool.", EssentialDoc: true},
		{Name: "MCP_MAX_SESSIONS_PER_PRINCIPAL", Group: "Transport", Default: "0", AppliesTo: []string{"streamable_http"}, Help: "Streamable HTTP session cap per authenticated subject+tenant on this replica (0=disabled; initialize requests beyond the cap return 429). Bounds blast radius when a single principal opens many initialize sessions.", EssentialDoc: true},
