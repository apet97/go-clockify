...
func AllSpecs() []EnvSpec {
	return []EnvSpec{
		...
		{Name: "MCP_TENANT_POLICY_CEILING", Group: "Safety", Enum: []string{"", "read_only", "time_tracking_safe", "safe_core", "standard", "full"}, AppliesTo: []string{"streamable_http", "grpc"}, EssentialDoc: true, Help: "Maximum policy mode a control-plane tenant record may select via TenantRecord.PolicyMode. Hosted profiles (shared-service, prod-postgres) default this to time_tracking_safe so a corrupted tenant row cannot broaden the process posture. Empty = no explicit ceiling; the process mode acts as the implicit ceiling. Ignored on stdio (no tenants). See docs/adr/0021-hosted-tenant-policy-ceiling.md."},
