...
// provider. The full JWT verify path runs once per token; subsequent
// requests with the same token take the verifyCache fast-path until the
// cache TTL (or the token's own exp claim, whichever is tighter)
// expires.
