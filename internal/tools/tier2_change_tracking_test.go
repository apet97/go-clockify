...
func TestEntityChangesSendsRepeatedTypesAndNormalizesTimeEntry(t *testing.T) {
	client, cleanup := newTestClient(t, func(w http.ResponseWriter, r *http.Request) {
		...
		if got := r.URL.Query()["type"]; !reflect.DeepEqual(got, []string{"TIME_ENTRY", "PROJECT"}) {
			t.Fatalf("type query = %#v, want repeated TIME_ENTRY/PROJECT", got)
		}
	...
	result, err := svc.entityChanges(context.Background(), map[string]any{
		"change_kind": "updated",
		"types":       []string{"TIME_ENTRY", "PROJECT"},
