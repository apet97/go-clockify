...
| 2026-05-18 | BUNDLE_YEAR_2024 sacrificial | `CLOCKIFY_RUN_LIVE_E2E=1`, `CLOCKIFY_LIVE_OPTIONAL_DOMAINS=1`, `CLOCKIFY_LIVE_PREFIX=MCP-LIVE-20260518*`, `CLOCKIFY_LIVE_WORKSPACE_CONFIRM=$CLOCKIFY_WORKSPACE_ID` | `TestOneUserLiveOptionalDomainContracts` extended to include `clockify_invoices_info` and `clockify_scheduling_publish` dry-run/recovery coverage | PASS (20.253s `internal/tools`) | optional-domain contract test may leave its existing prefixed optional objects; the new invoices-info read and scheduling-publish dry-run probe created no objects |
