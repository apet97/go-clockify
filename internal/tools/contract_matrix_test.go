...
func TestToolContractMatrix(t *testing.T) {
	...
	if len(all) != 137 {
		t.Fatalf("expected 137 tools, got %d", len(all))
	}
	...
	safeCoreWrites := map[string]bool{
		...
		"clockify_create_tag": true, "clockify_create_task": true, "clockify_delete_client": true, "clockify_delete_tag": true, "clockify_delete_task": true,
		"clockify_update_client": true, "clockify_update_tag": true, "clockify_update_task": true,
	}
