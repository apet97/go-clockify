...
func TestTier1CatalogGoldenCount(t *testing.T) {
	...
	reg := svc.Registry()
	if len(reg) != 41 {
		t.Fatalf("expected 41 Tier 1 tools, got %d", len(reg))
	}
}

// ---------------------------------------------------------------------------
// 10. Total tool count (Tier 1 + Tier 2 = 129)
// ---------------------------------------------------------------------------
...
func TestTotalToolCount(t *testing.T) {
	...
	total := tier1 + tier2
	if total != 129 {
		t.Fatalf("expected 129 total tools (41 Tier1 + 88 Tier2), got %d (%d + %d)", total, tier1, tier2)
	}
}
