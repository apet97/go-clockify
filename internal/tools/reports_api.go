...
func buildExpenseReportBody(args map[string]any) (map[string]any, error) {
	body := map[string]any{"exportType": "JSON"}
	setExpenseReportFields(body, args)
	setIfString(body, args, "note", "note")
	setIfBool(body, args, "without_note", "withoutNote")
	setIfInt64(body, args, "page", "page")
	setIfInt64(body, args, "page_size", "pageSize")
	setIfString(body, args, "sort_column", "sortColumn")
	if categories, ok, err := mapArg(args, "categories"); err != nil {
		return nil, err
	} else if ok {
		body["categories"] = normalizeContainsFilter(categories, false, false)
	}
	if _, ok := body["dateRangeType"]; !ok {
		if _, hasStart := body["dateRangeStart"]; hasStart {
			if _, hasEnd := body["dateRangeEnd"]; hasEnd {
...
func setExpenseReportFields(body map[string]any, args map[string]any) {
	...
	for _, spec := range []struct {
		...
		{"tasks", "tasks", false},
		{"user_groups", "userGroups", true},
		{"users", "users", true},
	} {
		if filter, ok, err := mapArg(args, spec.argKey); err == nil && ok {
			body[spec.bodyKey] = normalizeContainsFilter(filter, spec.userStatus, false)
		}
	}
}
