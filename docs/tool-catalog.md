...
| `clockify_projects_list` | `domain` | yes | no | yes | no | `read` | List projects in the pinned workspace, paginated via page and page_size. |
| `clockify_projects_create` | `domain` | no | no | no | no | `write` | Create a project in the pinned workspace. |
| `clockify_tasks_list` | `domain` | yes | no | yes | no | `read` | List tasks for a project, paginated via page and page_size. |
| `clockify_tasks_create` | `domain` | no | no | no | no | `write` | Create a task under a project. |
| `clockify_tags_list` | `domain` | yes | no | yes | no | `read` | List tags in the pinned workspace, paginated via page and page_size. |
...
| `clockify_users_list` | `domain` | yes | no | yes | no | `read` | List users in the pinned workspace, paginated via page and page_size. |
...
| `clockify_scheduling_assignments_get` | `domain` | yes | no | yes | no | `read` | Get a scheduling assignment by ID by scanning the supported date-range list endpoint page by page until found. If start/end are omitted, scans one year back through one year forward. |
| `clockify_scheduling_assignments_create` | `domain` | no | no | no | no | `write` | Create a recurring scheduling assignment for a user on a project. Retrying after a network timeout can create a duplicate assignment; check with clockify_scheduling_assignments_list before retrying. |
...
| `clockify_scheduling_project_totals` | `domain` | yes | no | yes | no | `read` | Get scheduling totals per project across a date range, paginated via page_size, with tracked amount/cost/profit comparison when Reports API enrichment is available |
| `clockify_approvals_list` | `domain` | yes | no | yes | no | `read` | List approval requests with optional status filter and pagination |
| `clockify_approvals_get` | `domain` | yes | no | yes | no | `read` | Get a single approval request by ID by scanning the documented approval-requests list endpoint page by page. |
...
| `clockify_webhooks_list` | `domain` | yes | no | yes | no | `read` | List webhooks in the workspace, paginated via page and page_size. |
| `clockify_webhooks_get` | `domain` | yes | no | yes | no | `read` | Get a webhook by ID from the pinned workspace. |
...
| `clockify_holidays_list` | `domain` | yes | no | yes | no | `read` | List holidays configured in the workspace; the full set is returned on a single page. |
| `clockify_holidays_list_for_user_period` | `domain` | yes | no | yes | no | `read` | List holidays assigned to a user across a date period; the full set is returned on a single page. |
...
| `clockify_projects_memberships_list` | `domain` | yes | no | yes | no | `read` | List project memberships read from the hydrated project record; the full membership set is returned on a single page. |
| `clockify_reports_attendance` | `domain` | yes | no | yes | no | `read` | Run the attendance report. Raw report amounts are in minor units (cents); meta.totalAmount gives normalized major-unit totals per currency. Large results truncate to the size cap. |
| `clockify_reports_money` | `domain` | yes | no | yes | no | `read` | Run money summary for billing-rate breakdowns by user or project. Use clockify_reports_summary for simple time totals. Raw amounts are minor units; meta.totalAmount gives major-unit totals. Large results truncate to the size cap. |
...
| `clockify_entries_timer_status` | `domain` | yes | no | yes | no | `read` | Show whether the current user has a running timer. |
...
| `clockify_api_get` | `raw` | yes | no | yes | no | `read` | Raw GET fallback for documented Clockify endpoints. Report/shared-report paths route to the reports host; others to api.clockify.me. Path must stay within /user or the pinned workspace; other workspaces/hosts rejected. |
| `clockify_api_request` | `raw` | no | no | no | yes | `write` | Raw method fallback for documented Clockify endpoints. Report/shared-report paths route to the reports host; others to api.clockify.me. Path must stay within /user or the pinned workspace. Raw PATCH to reports paths is unsupported. |
