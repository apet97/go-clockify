...
High-risk live execution now requires a short-lived `confirm_token` from the
matching dry run; central previews are available even when a handler has no
custom preview body.
...
| `clockify_invoices_import_time` | billing | Imports billable time into an invoice. | yes |
| `clockify_invoices_import_expenses` | billing | Imports billable time and expenses into an invoice. | yes |
| `clockify_invoices_payments_create` | billing | Creates an invoice payment. | yes |
...
| `clockify_expenses_update` | billing | Updates an expense multipart body. | yes |
...
| `clockify_custom_fields_create` | admin | Creates a custom field. | yes |
| `clockify_custom_fields_update` | admin | Updates a custom field. | yes |
| `clockify_custom_fields_delete` | admin, destructive | Deletes a custom field. | yes |
| `clockify_custom_fields_set_value` | admin | Sets a project or time-entry custom-field value. | yes |
...
| `clockify_time_off_approve` | admin, permission change | Approves a pending time-off request. | yes |
| `clockify_time_off_deny` | admin, permission change | Denies a pending time-off request. | yes |
| `clockify_time_off_policies_create` | admin | Creates a simplified time-off policy. | yes |
| `clockify_time_off_policies_update` | admin | Updates a time-off policy by merging supplied fields. | yes |
| `clockify_time_off_archive` | admin, destructive | Archives or reactivates a time-off policy. | yes |
...
| `clockify_approvals_resubmit` | admin, permission change | Resubmits rejected or withdrawn entries and expenses. | yes |
...
| `clockify_groups_create` | admin | Creates a user group. | yes |
| `clockify_groups_update` | admin | Updates a user group. | yes |
...
| `clockify_entries_mark_invoiced` | billing | Marks time entries invoiced or not invoiced. | yes |
