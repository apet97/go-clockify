...
| `MCP_EXPOSE_AUTH_ERRORS=0` (profile default) | Keeps client auth failures generic. Hosted profiles refuse `MCP_EXPOSE_AUTH_ERRORS=1` unless `MCP_EXPOSE_AUTH_ERRORS_BREAK_GLASS=1` is also set, and then emit a `hosted_break_glass_enabled` startup warning. | Detailed authenticator errors can reveal issuer, audience, tenant-claim, proxy, or token-shape drift to clients. |
...
| `MCP_TENANT_POLICY_CEILING=time_tracking_safe` | Cap the policy mode any control-plane tenant record (`TenantRecord.PolicyMode`) may select. Tenants can narrow further (e.g. `read_only`) but cannot broaden to `safe_core`, `standard`, or `full`. Tenant deny lists union with the process denies; tenant allow lists intersect. See [ADR 0021](../adr/0021-hosted-tenant-policy-ceiling.md) and [`docs/policy/production-tool-scope.md` § Tenant policy ceiling](../policy/production-tool-scope.md#tenant-policy-ceiling-multi-tenant-hosted-deployments). | A corrupted Postgres tenant row or an operator pushing `PolicyMode: standard` to one tenant could silently broaden the live posture, re-opening Tier-2 admin surface and group elevation. The ceiling fails session-create closed when a tenant exceeds it. |
...
| `CLOCKIFY_WEBHOOK_VALIDATE_DNS_BREAK_GLASS=0` (default) | Hosted profiles refuse `CLOCKIFY_WEBHOOK_VALIDATE_DNS=0` unless this incident-only variable is set, and then emit a `hosted_break_glass_enabled` startup warning. Prefer `CLOCKIFY_WEBHOOK_ALLOWED_DOMAINS` for split-horizon allow-listing. | Disabling DNS validation globally reopens webhook private-IP SSRF exposure for every hostname. |
