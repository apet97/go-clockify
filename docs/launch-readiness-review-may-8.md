...
| Decide whether the objective is actually complete. | Completion requires no missing objective requirement. Group 1 scheduled final-SHA evidence is closed by scheduled runs 25608259477 and 25607242862; Group 6 candidate-tag security evidence is closed on `v1.2.1-rc.2` (peeled `d83f9f86d3b95594abef2ee035554510faa799c1`, evidence in [`SECURITY.md`](../SECURITY.md#candidate-tag-security-evidence)); Group 7 release/sigstore/SLSA evidence (rc.2 Release / Docker Image / Deploy / release-smoke / npm-publish), pushed workflow evidence where still missing, mutation cron evidence on the candidate SHA, restoring the historical 19-context branch protection (issue #78) as a follow-up to the 2026-05-09 repository-state cleanup, hosted quota evidence, and legal/product approval are still missing. | **Not complete. Do not mark launch-ready.** |
...
| Keep Group 6 security posture verifiable. | `docs/launch-candidate-checklist.md`; `docs/runbooks/release-candidate-evidence.md`; `scripts/prepare-rc-evidence.sh`; [`SECURITY.md`](../SECURITY.md#candidate-tag-security-evidence). | Candidate-tag evidence on `v1.2.1-rc.2` (peeled `d83f9f86d3b95594abef2ee035554510faa799c1`, captured 2026-05-09T22:24Z–22:26Z UTC from a clean worktree on FIPS-capable darwin/arm64): `GOTOOLCHAIN=go1.25.10 make {check,verify-vuln,secret-scan,verify-fips}` exit 0; pinned `govulncheck@v1.3.0` reports `No vulnerabilities found.` under `Go: go1.25.10` against vuln DB `2026-05-07 19:21:40 UTC`; `make secret-scan` and gitleaks `8.30.1` report `no leaks found` from a clean worktree at the candidate-tag tree (the prior parent-workstation `make secret-scan` skip was caused by ignored `.local/`, `.serena/`, and duplicate `go-clockify/` artifacts outside the candidate-tag tree, so the "clean candidate-tag gitleaks remains required" preflight gate is now satisfied by this isolated worktree run); Semgrep `p/default` (semgrep `1.157.0`) scanned 1155 tracked files with 0 findings via `semgrep scan --config p/default --metrics=off --error --exclude .git --exclude .bench --exclude clockify-mcp .`; the five production `nosemgrep` directives all carry inline justifications and map to ADR 0017 (SSE direct writes) or ADR 0008 (bufconn test transport); `make verify-fips` ran `-tags=fips` and `-tags=fips,grpc` green. A host-toolchain govulncheck scan with Go 1.26.2 reports standard-library issues fixed in Go 1.26.3, so README/CONTRIBUTING keep the exact Go 1.25.10 launch-candidate pin instead of broad `1.25.10+` wording. The four candidate-tag-bounded Group 6 boxes are ticked in `docs/launch-candidate-checklist.md`; the three profile-defaults boxes were already closed. | Group 6 closed on `v1.2.1-rc.2`; Group 7 still open. |
...
| Decide completion from real evidence only. | This checklist; `make launch-external-status`; `make public-content-audit`; `git status --short --branch`; [`SECURITY.md`](../SECURITY.md#candidate-tag-security-evidence). | The tree is landed on `main` and locally coherent, Group 1 scheduled-cron launch evidence is archived, and Group 6 candidate-tag security evidence is now archived for `v1.2.1-rc.2` (peeled `d83f9f86d3b95594abef2ee035554510faa799c1`). Group 7 release/sigstore/SLSA/npm publish evidence on rc.2, mutation cron evidence on the candidate SHA, restoring the historical 19-context branch protection (issue #78), and legal/product approval remain incomplete. | **Not complete. Do not mark launch-ready.** |
...
- **Group 6 candidate-tag security walk-through.** _Closed 2026-05-10
  on `v1.2.1-rc.2` (peeled `d83f9f86d3b95594abef2ee035554510faa799c1`):
  `GOTOOLCHAIN=go1.25.10 make {check,verify-vuln,secret-scan,verify-fips}`
  and `semgrep scan --config p/default --metrics=off --error --exclude
  .git --exclude .bench --exclude clockify-mcp .` (1155 tracked files,
  0 findings) all exit 0 from a clean worktree at the candidate-tag
  tree. Pinned `govulncheck@v1.3.0` reported `No vulnerabilities found.`
  under `Go: go1.25.10`; gitleaks `8.30.1` reported `no leaks found`;
  `nosemgrep` directives all map to ADR 0008 (bufconn test transport)
  or ADR 0017 (SSE direct writes). Evidence block in
  [`SECURITY.md`](../SECURITY.md#candidate-tag-security-evidence) and
  the four candidate-tag-bounded Group 6 boxes in
  [`launch-candidate-checklist.md`](launch-candidate-checklist.md#6-security-and-policy-review)
  are now ticked. This closes `L-04`. Group 7 (release/sigstore/SLSA/npm
  publish), mutation-cron evidence on the candidate SHA, and the
  hosted/legal/product gates remain open._
