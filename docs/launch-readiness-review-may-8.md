...
| Decide whether the objective is actually complete. | Completion requires no missing objective requirement. Group 1 scheduled final-SHA evidence is closed by scheduled runs 25608259477 and 25607242862; Group 6 candidate-tag security evidence is closed for `v1.2.1-rc.3` on peeled commit `ce56414ae012c4a49d21ae0a319b178619c5966a` (transcript in [`SECURITY.md`](../SECURITY.md) § "Candidate-tag security evidence"); Group 7 release/sigstore/SLSA evidence (separate lane), pushed workflow evidence where still missing, mutation cron evidence, repository-state cleanup, hosted quota evidence, the npm expected-version proof, issue #78 (19-context branch-protection restoration), and legal/product approval are still missing. | **Not complete. Do not mark launch-ready.** |
...
| Keep Group 6 security posture verifiable. | `docs/launch-candidate-checklist.md`; `docs/runbooks/release-candidate-evidence.md`; `scripts/prepare-rc-evidence.sh`; [`SECURITY.md`](../SECURITY.md) § "Candidate-tag security evidence". | Earlier preflight context: pinned `govulncheck@v1.3.0` found no vulnerabilities under `GOTOOLCHAIN=go1.25.10`; Semgrep `p/default` scanned 1153 tracked files with 0 findings; `nosemgrep` context maps to ADR 0008 / ADR 0017; `make verify-fips` passed. A host-toolchain govulncheck scan with Go 1.26.2 reported standard-library issues fixed in Go 1.26.3, so README/CONTRIBUTING keep the exact Go 1.25.10 launch-candidate pin. The 2026-05-09 preflight noted that `make secret-scan` would not be green on a dirty workstation because ignored/local artifacts remained, so a `clean candidate-tag gitleaks remains required` note was carried into rc evidence work. The `v1.2.1-rc.3` candidate-tag walk-through closed that requirement on 2026-05-10 from peeled commit `ce56414ae012c4a49d21ae0a319b178619c5966a` (host short name `192`, fresh worktree with no `.local/`, `.serena/`, or duplicate `go-clockify/` checkouts): `make check` (exit 0), `make verify-vuln` (`govulncheck@v1.3.0` under `GOTOOLCHAIN=go1.25.10`, vuln.go.dev DB 2026-05-07 19:21:40 UTC, no vulnerabilities), `make secret-scan` (`gitleaks 8.30.1`, no leaks), `semgrep 1.157.0 scan --config p/default --metrics=off --error --exclude .git --exclude .bench --exclude clockify-mcp .` (558 rules across 1155 tracked files, 0 findings), `git grep -n -C 5 nosemgrep -- ':!CHANGELOG.md'` (5 in-source directives, all justified inline within 5 lines and traced to ADR 0008 / ADR 0017), and `make verify-fips` (`-tags=fips` GOFIPS140=latest `fips140_enabled` plus `-tags=fips,grpc`) all green; full transcript in [`SECURITY.md`](../SECURITY.md) § "Candidate-tag security evidence". | Closed for `v1.2.1-rc.3`; future candidate tags must re-run the same suite. |
...
| Decide completion from real evidence only. | This checklist; `make launch-external-status`; `make public-content-audit`; `git status --short --branch`; [`SECURITY.md`](../SECURITY.md) § "Candidate-tag security evidence". | The tree is landed on `main` and locally coherent, Group 1 scheduled-cron launch evidence is archived, and Group 6 candidate-tag security evidence is closed for `v1.2.1-rc.3` on peeled commit `ce56414ae012c4a49d21ae0a319b178619c5966a`. Group 7 release/sigstore/SLSA evidence (separate lane), mutation cron evidence, external repo-state cleanup, the npm expected-version proof, issue #78, and legal/product approval remain incomplete. | **Not complete. Do not mark launch-ready.** |
...
- **Group 6 candidate-tag security walk-through.** Closed for
  ...
  25616879075, Reproducibility run 25616925376 (all 9 matrix jobs
  match released bytes), and release-smoke run 25616925600 are green
  on rc.3. A fresh worktree off the peeled commit on host short name
  `192` (macOS arm64) ran `make check` (exit 0), `make verify-vuln`
  (exit 0; `govulncheck@v1.3.0` under `GOTOOLCHAIN=go1.25.10` against
  the `vuln.go.dev` DB updated 2026-05-07 19:21:40 UTC reported `No
  vulnerabilities found.`), `make secret-scan` (exit 0; `gitleaks
  8.30.1` reported `no leaks found` after scanning ~4.97 MB),
  `semgrep scan --config p/default --metrics=off --error --exclude
  .git --exclude .bench --exclude clockify-mcp .` (exit 0; `semgrep
  1.157.0` ran 558 rules over 1155 git-tracked files with `Findings:
  0 (0 blocking)`), `git grep -n -C 5 nosemgrep -- ':!CHANGELOG.md'`
  (5 in-source directives — `tests/harness/grpc.go:71` mapped to
  ADR 0008 and `internal/mcp/transport_streamable_http.go:541,563,565,568`
  mapped to ADR 0017, each with inline justification within five
  lines), and `make verify-fips` (exit 0; `-tags=fips` GOFIPS140=latest
  emitted `INFO fips140_enabled` then every package test passed,
  `-tags=fips,grpc` build green). The full transcript lives in
  [`SECURITY.md`](../SECURITY.md) § "Candidate-tag security evidence",
  and the four candidate-tag-dependent boxes in the Group 6 row of
