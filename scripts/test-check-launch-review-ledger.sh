...
mut_stale_doc_parity_case_count() {
  perl -0pi -e 's/current doc-parity regression suite has 70 cases/current doc-parity regression suite has 65 cases/' "$1"
}
MUTATOR=mut_stale_doc_parity_case_count
run_case "stale doc-parity case count fails closed" \
  1 'prompt-to-artifact checklist missing required text.*70 cases'
