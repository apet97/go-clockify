...
func writeMarkdown(path string, c catalog) error {
	...
	fmt.Fprintf(&b, "- `tools/list` returns the advertised toolset, not the loaded registry. The default toolset advertises 16 everyday tools; `CLOCKIFY_TOOLSET=all` advertises all %d startup tools; unadvertised tools remain dispatch-callable by name. No cursor pagination.\n\n", len(c.Tools))
