...
# a flat generated catalog with 154 tools, minimal env surface, current
...
EOF
  ...
  write_catalog "$dir" 154

  cat > "$dir/README.md" <<'EOF'
# go-clockify

One-user local Clockify MCP with 154 tools.
...
The local stdio package exposes 154 tools.
EOF

  cat > "$dir/docs/api-coverage.md" <<'EOF'
# API Coverage

The generated catalog currently exposes 154 tools.
EOF
...
count_drift() {
  perl -0pi -e 's/154 tools/150 tools/' "$1/README.md"
}
...
missing_agents_contract() {
  grep -v 'Exactly 154 tools loaded at startup' "$1/AGENTS.md" > "$1/AGENTS.tmp"
...
run_case "flat catalog must stay at 154 tools" 1 "tool count drift: found 150 tools, expected 154" bad_catalog_count
