...
Preconditions:
  ...
  - govulncheck, gitleaks, semgrep, gh, and FIPS-capable Go tooling are installed.
...
EOF
    for wf in "${workflow_files[@]}"; do
        printf '  - latest runs for %s via gh run list\n' "$wf"
    done
    cat <<EOF
...
Next required human steps:
  ...
  3. Run docs/verification.md checks for required variants not sampled
     by release-smoke.yml, including the Group 7 FIPS variant evidence.
  4. Add evidence URLs / workflow_run_id values to Groups 6 and 7 in
     docs/launch-candidate-checklist.md before checking boxes.
  5. Run make launch-checklist-parity after editing the checklist.
EOF
